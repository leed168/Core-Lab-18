...
public class InkScript1 : MonoBehaviour 
{
	...
	IEnumerator RefreshView () 
    {
		...
		RemoveChildren ();
        yield return new WaitForSeconds(0.5f); // Pauses for a second
	...
	void RemoveChildren () 
    {
        ...
        foreach (Graphic g in gfx)
        {
            g.CrossFadeAlpha(0, 0.5f, true);
              ...
              {
                  Destroy(canvas.transform.GetChild (i).gameObject, 0.5f); // 0.5f adds delay
        }
    }
